...
## The EXAMINER hook: a helper that catches messy code as you write it

EXAMINER (step 4) is something you run by hand. This hook does one slice of its job for you, on its own.

**What it does, in one line:**
Every time Claude Code changes a Python file, the hook checks it. If the change looks messy, it tells Claude right away. So the mess gets fixed now, not months later.

**What counts as messy?** Two things for now:

- **A "catch everything" error handler.** Code that hides every error instead of dealing with it. The bug still happens. You just cannot see it.
- **A function that got too big.** One that quietly does several jobs at once, which makes it risky to change.

**When your code is clean, it says nothing.**
No noise. It never stops your edit and never undoes it. It only points. If it points at the wrong thing, ignore it and move on.

This "let a simple checker point, let the AI react" idea comes from Ivett Ördög's Habit Hooks. Credit below.

It checks Python only for now, because it uses a Python checker called ruff. The same idea works with other checkers later.

### Turn it on

This repo already has it set up. Clone it, open it in Claude Code, say yes when it asks to approve the hook, and it works.

To add it to your own project, four steps:

**1. Install ruff.** This is the checker the hook uses.
```
pip install ruff
```

**2. Copy the folder.** Put the `examiner-hook/` folder into your project.

**3. Add a settings file.** Create `.claude/settings.json` with this inside:
```json
{
  "hooks": {
    "PostToolUse": [
...
**4. Turn it on.** Open Claude Code in your project and approve the hook when it asks. Then edit a Python file that has one of the two messy patterns above, and watch the tip show up on its own.

### Try it by hand first

You do not need Claude Code to see it work. Run the checker on any Python file:
```
python3 examiner-hook/examiner_hook.py path/to/file.py
```
It prints what it found and the tip for each one. The file `examiner-hook/demo_messy_example.py` is a good one to test on.
