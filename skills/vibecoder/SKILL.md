...
Web content injection check: if the project fetches external web content that an AI reads or reasons over, check whether that content is treated as untrusted data at every stage of the pipeline. Instructions can be embedded in HTML comments, script tags, or third-party plugin output. These are invisible to human users but readable by AI tools. Check specifically for: any fetched content entering an agent context without sanitisation first, any code path where fetched content could instruct the AI to take an action or conceal something from the user, any third-party scripts or plugins that load external content into an AI-readable context. Flag any of these as HIGH or CRITICAL depending on what the AI is authorised to do. (Credit: Matthew Sutherland, ByteFlowAI — caught two live injections in a client audit using this pattern.)
...
- CRITICAL: exposed secrets, injection, auth bypass, data exposure, AI acting on concealment instructions from fetched content

- HIGH: missing validation, permissive CORS, debug mode, prompt injection, fetched web content entering agent context without sanitisation
...
Replace [project-root] with the root folder of the project being scanned. Replace [YYYYMMDD] with today's date. Create the reviews/ directory if it does not exist.
...
0. BRAINSTORM: "What are we actually building?" Idea to brief.

1. CODEMAKER: "Let me build it." Greenfield build.

2. VIBECODER: "Here is what you have." Scan and document — this step.
